...
class Project(models.Model):
    ...
    cors_allowed_origins = models.JSONField(
        default=list,
        help_text="List of allowed origins for CORS, or ['*'] for all"
    )
    ...
    def should_sample_event(self, event_source=None, user_id=None):
        ...
        if strategy == 'random':
            ...
            # Consistent sampling based on user_id hash
            hash_val = int(hashlib.md5(f"{user_id}_{self.id}".encode()).hexdigest(), 16)
...
class EventSource(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name='event_sources')
