...
class EventAggregation(models.Model):
    ...
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="%(class)s_aggregations"
    )
...
class FiveMinuteEventAggregation(EventAggregation):
    ...
    datetime_5min = models.DateTimeField(
        db_index=True
    )  # Truncated to 5-minute intervals
...
class ProjectDailySummary(models.Model):
    ...
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="daily_summaries"
    )
    date = models.DateField(db_index=True)
